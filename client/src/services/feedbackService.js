...
export const feedbackService = {
  ...
  async getFeedbackByUser(userId, filters = {}) {
    try {
      ...
      const response = await api.get(`/feedback/user/${userId}?${params.toString()}`);
      return {
        success: true,
        data: response.data.data,
      };
      ...
      return {
        success: false,
        message: error.response?.data?.message || 'Failed to fetch user feedback',
        error: error.response?.data?.error || error.message,
      };
    }
  },
  ...
  async getFeedbackStats() {
    try {
      ...
      return {
        success: false,
        message: error.response?.data?.message || 'Failed to fetch feedback stats',
        error: error.response?.data?.error || error.message,
      };
    }
  }
