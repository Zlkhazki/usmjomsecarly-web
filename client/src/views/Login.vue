...
// Watch for changes in formData.email and log them
watch(
  () => formData.email,
  (newValue, oldValue) => {
    console.log(`formData.email changed from "${oldValue}" to "${newValue}"`);
  }
);
...
const handleLogin = async () => {
  console.log(
    "Inside handleLogin, before validateForm. formData.email:",
    formData.email
  ); // Log before validation
  if (!validateForm()) {
    console.log("Form validation failed. formData.email:", formData.email); // Log if validation fails
    return;
  }
  console.log(
    "Inside handleLogin, after validateForm. formData.email:",
    formData.email
  ); // Log after validation

  loading.value = true;
  errors.form = ""; // Clear previous form errors

  const payload = {
    email: formData.email,
    password: formData.password,
  };
  console.log("Payload to be sent by Axios:", JSON.stringify(payload)); // Log the exact payload
  ...
  try {
    ...
    if (response.data.success && response.data.token) {
      ...
      errors.form = response.data.message || "Login failed. Please try again.";
    }
  } catch (error) {
...
<template>
  <div
    class="flex items-center justify-center min-h-screen bg-gradient-to-b from-[#330b4f] to-[#4a1572] bg-cover bg-no-repeat bg-center"
  >
    <div
      class="relative flex flex-col m-6 space-y-8 md:w-3/4 lg:w-3/5 bg-white shadow-2xl rounded-2xl lg:flex-row lg:space-y-0"
    >
      ...
      <div class="relative lg:w-1/2">
        ...
        />
        <div
          class="absolute inset-0 bg-gradient-to-t from-[#330b4f] to-transparent opacity-70 rounded-l-2xl hidden lg:block"
        ></div>
      ...
      <div class="flex flex-col lg:w-1/2 justify-center p-8 lg:p-14">
        ...
        <form @submit.prevent="handleLogin" class="space-y-6">
          ...
          <div class="field">
            ...
            />
            <small class="p-error block mt-1" v-if="errors.email">{{
              errors.email
            }}</small>
          </div>
          ...
          <div class="flex items-center justify-between">
            ...
            <a
              href="#"
              class="text-sm text-[#330b4f] hover:text-[#4a1572] font-medium"
              >Forgot Password?</a
            >
          </div>
        ...
        <div class="mt-10 text-center text-gray-500 text-sm">
          &copy; {{ new Date().getFullYear() }} Jom SeCarly. All rights
          reserved.
        </div>
      </div>
    </div>
